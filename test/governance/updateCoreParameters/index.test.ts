...
contract('Governance', (accounts) => {
    ...
    describe('proposeUpdateCoreParameters', () => {
        it('works well', async () => {
            ...
            try {
                const res = await proposeUpdateCoreParameters(proposer, newParameters, {
                    governanceInstance,
                    taxTokenInstance,
                });
        ...
        it('reverts when preVoteLength is more than totalVoteLength', async () => {
            const lendingTokenInstance = await TestToken.new(
                'Test ERC20',
                'TEST',
                decimalsOfLendingToken
            ...
            const newParameters = {
                ...initialCoreParameters,
                preVoteLength: 0, // = 7 * DAYS
                totalVoteLength: 6 * DAYS,
            };
            ...
            try {
                await proposeUpdateCoreParameters(proposer, newParameters, {
                    governanceInstance,
                    taxTokenInstance,
                });
            } catch (err) {
            ...
            assert.ok(`an error was expected, but it was successful: ${errorMessage}`);
        });
    });

    describe('applyGovernanceForUpdateCore', () => {
        const initialTaxTokenSupplyE0 = 20000;
    ...
    describe('getInfoUpdateCoreParameters', () => {
        const initialTaxTokenSupplyE0 = 20000;
        ...
        it('returns correct values', async () => {
            ...
            const { proposeId } = await proposeUpdateCoreParameters(proposer, newParameters, {
                governanceInstance,
                taxTokenInstance,
            });

            const parametersProposal = await getInfoUpdateCoreParameters(proposeId, {
                governanceInstance,
            });
