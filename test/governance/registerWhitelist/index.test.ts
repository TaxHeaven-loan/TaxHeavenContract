...
import {
    proposeRegisterWhitelist,
    applyRegisterWhitelistProposal,
    getInfoRegisterWhitelist,
} from './util';
...
import { applyIncentiveFundProposal } from '../updateIncentiveAddresses/util';
import type {
    GovernanceInstance,
    LendingInstance,
    TaxTokenInstance,
...
contract('Governance', (accounts) => {
    ...
    describe('proposeUpdateWhitelist', () => {
        it('works well', async () => {
            ...
            const newParameters = {
                ...
                oracleAddress: lendingTokenPriceOracleInstance.address,
            };
            ...
            try {
                const res = await proposeRegisterWhitelist(proposer, newParameters, {
                    governanceInstance,
                    taxTokenInstance,
                });
        ...
        it('reverts when a proposer try to register a zero address as oracle', async () => {
            const lendingTokenInstance = await TestToken.new(
                'Test ERC20',
                'TEST',
                decimalsOfLendingToken
            ...
            const newParameters = {
                tokenAddress: ZERO_ADDRESS,
                oracleAddress: '0x' + '1'.padStart(40, '0'), // address(0x000...001)
            };
            ...
            try {
                await proposeRegisterWhitelist(proposer, newParameters, {
                    governanceInstance,
                    taxTokenInstance,
                });
            } catch (err) {
    ...
    describe('applyGovernanceForUpdateWhitelist', () => {
        const initialTaxTokenSupplyE0 = 20000;
        ...
        let lendingTokenPriceOracleInstance: TestOracleInstance;
        ...
        beforeEach(async () => {
...

            {
                ...
                lendingTokenPriceOracleInstance = res.lendingTokenPriceOracleInstance;
            }
    ...
    describe('getInfoUpdateWhitelist', () => {
        const initialTaxTokenSupplyE0 = 20000;
        ...
        it('returns correct values', async () => {
            ...
            const { proposeId } = await proposeRegisterWhitelist(proposer, newParameters, {
                governanceInstance,
                taxTokenInstance,
            });

            const parametersProposal = await getInfoRegisterWhitelist(proposeId, {
                governanceInstance,
            });
